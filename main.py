...
from utils import draw_text, create_explosion, load_high_score, save_high_score
...
enemies = spawn_enemies(assets["enemy_img"], SCREEN_WIDTH)
...
while running:
    ...
    # --- STATE: GAME ---
            ...
            if b.bottom < 0: bullets.remove(b)
                ...
                for e in enemies[:]:
                    if b.colliderect(e.rect):
                        ...
                        score_value += 10
                        assets["explosion_sound"].play()
                        if random.random() < 0.02: # Drop PowerUp
        ...
        for pw in powerups[:]:
            ...
            pygame.draw.circle(screen, color, pw["rect"].center, 12)
            pygame.draw.circle(screen, WHITE, pw["rect"].center, 12, 2)

            if pw["rect"].colliderect(player.rect):
                if pw["type"] == "life": player.lives += 1
                elif pw["type"] == "shield":
                    player.last_hit_time = current_time + 3000 # Extend invulnerability by 3 seconds
                assets["explosion_sound"].play()
                powerups.remove(pw)
        ...
        draw_text(screen, f"Score: {score_value}", assets["score_font"], WHITE, 100, 50)
        draw_text(screen, f"Level: {current_level}", assets["score_font"], YELLOW, SCREEN_WIDTH // 2, 50) 
        draw_text(screen, f"Lives: {player.lives}", assets["score_font"], RED, 100, 90)
